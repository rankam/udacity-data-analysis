...
def add_salary_estimate(df):
    ct = [1000,10000,25000,50000,75000,100000] # Counter to keep me informed on what iteration its on
    df['estimated_salary'] = np.nan
    errors = list()
    for i in range(len(df)):
        if i in ct:
            print i # Counter to keep me informed on what iteration its on
        try:
