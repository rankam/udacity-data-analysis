...
Our logistic regression model is not predicting if the contribution was made to Democrats or if the contribution was made to Republicans. Instead, it is only predicting if the contribution was made to Democrats. If the model predicts that the contribution was not made to Democrats, then the prediction will be the other choice which is Republicans in our case. 
...
ggplot(roc.df,aes(specificities, sensitivities)) + geom_line(aes(), size=1.5) + xlim(1,0) + xlab('Specificity - False Positive Rate') + ylab('Sensitivity - True Positive Rate') + annotate("text",x=.8,y=0.95,label=paste('The Area Under the Curve is ',auc))

ggplot(roc.df,aes(s, sensitivities)) + geom_line(aes(), size=1.5) + xlab('Specificity - False Positive Rate') + ylab('Sensitivity - True Positive Rate') + annotate("text",x=.8,y=0.95,label=paste('The Area Under the Curve is ',auc))
```

####Cross-validation
```{r, message=F, warning=F}
cv.fit <- cv.glm(glm.data, fit, K=10)
