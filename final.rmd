...
```{r, message=F, warning=F}
# Gets days from general election day
...
data$contbr_fnm <- getFirstName(data$contbr_nm)
...
for ( x in rows) {
  if (x %in% names(t)) {
    t[x] <- as.numeric(t[x]) + 1
    }
  else {
    t[x] <- 1
    }
  }
test <- sapply(1:length(rows), function(i) {
  if (t[rows[i]] > 1) {
    return(1) 
  } 
  else {
...
```{r, echo=FALSE}
ggplot(aes(data$population), data=data) + geom_histogram() + xlim(0,quantile(data$population, .95))
...
qplot(data$elect_delta, fill=data$election_tp) #ELECTION TYPE
```
...
###Do Democrats or Republicans Receive Higher Donation Amounts?
```{r, message=F, warning=F}
data <- subset(data, cand_party == 'D' | cand_party == 'R' )
data.by.party <- groupData(data,cand_party)
ggplot(data,aes(1, log(contb_receipt_amt+1))) + geom_boxplot(aes(fill=cand_party))

mann.fit<-wilcox.test(subset(data, cand_party == 'R')$contb_receipt_amt, subset(data, cand_party == 'D')$contb_receipt_amt)
```
Looking at the boxplot, it appears that contributions to Republicans are of a higher amount than those made to Democrats. To help confirm my visual assumptions, I performed a Mann-Whitney U Test which is a nonparametric test for comparing two populations. The question the Mann-Whitney U Test was answering is:
  - Do the two populations (Contributions made to Republicans and contributions made to Democrats) have identical distributions?
Looking at the results of the test, I see a p-value < 2.2e-16 so I can conclude that the there is a statistically significant difference between the two populations. Thus, I can answer my question - Contributions made to Republicans do appear to higher than those made to Democrats.


###Do Democrats or Republicans Receive More Donations?
```{r, message=F, warning=F}
qplot(data$cand_party, geom='bar')
table(data$cand_party)
```
Democrats received nearly 3 times more donations than Republicans. 


###Who Donates More - Men or Women?
```{r, message=F, warning=F}
qplot(data$predicted_gender, data$contb_receipt_amt, geom='bar', stat='identity')
ggplot(data, aes(predicted_gender, log(contb_receipt_amt +1))) + geom_boxplot() + facet_wrap(~cand_party)
```

###Who Donates More Frequently - Men or Women?
```{r, message=F, warning=F}
qplot(data$predicted_gender, geom='bar')
ggplot(data, aes(x=predicted_gender)) + geom_bar() + facet_wrap(~cand_party)
```


###Do people with higher estimated salaries donate more than those with lower estimated salaries?
```{r, message=F, warning=F}
ggplot(data, aes(estimated_salary, contb_receipt_amt)) + geom_point() + xlim(0,quantile(data$estimated_salary, .95)) + ylim(0, quantile(data$contb_receipt_amt, .95)) + geom_smooth(method='loess')


```

###Do people with higher estimated salaries donate to Ds or Rs?
```{r, message=F, warning=F}
data <- subset(data,cand_party=='D' | cand_party == 'R')
ggplot(data, aes(estimated_salary)) + geom_histogram() + facet_wrap(~ cand_party) 
ggplot(subset(data,cand_party=='D' | cand_party == 'R'), aes(1,log(estimated_salary+10))) + geom_boxplot(aes(fill=cand_party))
```
This histogram reveals distributions similar to what I expected - salaries in the United States often follow a an exponential or log-normal distribution though it does not give a clear indication if there is a relationship between estimated salary and which political party one conbtributes to. The boxplot appears to show that people with higher salaries give to Republicans, but it's far from conclusive. 

###Do people donate more when it is closer to the election?
```{r, message=F, warning=F}
data.elect <- data.frame(groupData(subset(data,cand_party=='D' | cand_party == 'R'), elect_delta,cand_party))
ggplot(data.elect, aes(elect_delta, median_contb)) + geom_line() + facet_grid(~cand_party) + geom_smooth()
...
###Do people make donations on a certain day?
```{r, message=F, warning=F}
data <- subset(data, cand_party == 'D' | cand_party == 'R')
...
ggplot(data,aes(weekday)) + geom_bar() + facet_wrap(~ cand_party)


ggplot(data,aes(1,contb_receipt_amt, fill=cand_party)) + geom_boxplot() + ylim(0, quantile(data$contb_receipt_amt, .95)) + facet_grid(~weekday)

data.weekday <- groupData(data, weekday, cand_party)
qplot(data=data.weekday, weekday, mean_contb, geom='bar', stat='identity', fill=cand_party)

data.month <- groupData(data,yearmon, cand_party)

ggplot(data, aes(yearmon, contb_receipt_amt))  + scale_x_yearmon() +stat_summary(aes(color=cand_party), geom='line',fun.y=mean) + ylim(0, quantile(data$contb_receipt_amt, .95))


ggplot(data, aes(yearmon, contb_receipt_amt))  + scale_x_yearqtr() +stat_summary(aes(color=cand_party), geom='line',fun.y=mean) + ylim(0, quantile(data$contb_receipt_amt, .95)) + facet_wrap(~predicted_gender)
```

###Do people donate more frequently when it is closer to the election?
```{r, message=F, warning=F}
...
###Do people that live in cities donate more or less?
```{r, message=F, warning=F}
ggplot(data, aes(1,log(contb_receipt_amt+10))) + geom_boxplot() + facet_wrap(~ popbins) + ylim(quantile(log(data$contb_receipt_amt+10), .05),quantile(log(data$contb_receipt_amt+10), .95))
```
...
```{r, message=F, warning=F}
data.party.city <- groupData(data, cand_party, contbr_city)


data.party.elect <- groupData(data, cand_party,elect_delta)

ggplot(aes(x=elect_delta,y=mean_contb), data=data.party.elect) + geom_point(aes(color=cand_party)) + scale_x_discrete(breaks = seq(0, 648, 30)) + geom_smooth(method='loess') + facet_wrap(~ cand_party)
ggplot(aes(x=elect_delta/7,y=mean_contb), data=data.party.elect) + geom_line(aes(color=cand_party))
...
data.cand_party.elect_delta <- groupData(data, elect_delta,cand_party)

ggplot(data.cand_party.elect_delta, aes(elect_delta, sqrt(sum_contb), color=cand_party)) + geom_point() 

ggplot(data.cand_party.elect_delta, aes(elect_delta, median_contb, color=cand_party)) + geom_point()

data.cand_party.include_gender <- groupData(data, included_gender,cand_party)

ggplot(data.cand_party.include_gender,aes(included_gender,count)) + geom_bar(stat='identity') + facet_wrap(~ cand_party)                                                                                                            
```
...
In our ROC curve, we will see the proportion correctly identified as Democrats plotted against the proportion incorrectly identified as Democrats. 
 
 Does the model do a good job of detecting those contributions made to Democrats
...
While it is possible to provide specific details about the truth and false positive rates, it should suffice to say that we can think of the truth positive rate as correctly classified and the false positive rate as incorrectly classified. Plotting the true positive rate against the false positive rate will show the ROC curve and by calculating the area under this curve (AUC), we are able to evaluate the performance of our model. The more area under the ROC curve indicates greater predictive power: 
 ...
 The closer the curve follows the left-hand border and then the top border of the ROC space, the more accurate the test (http://gim.unmc.edu/dxtests/roc2.htm)
...
roc.plot <- ggplot(roc.df,aes(1-specificities, sensitivities)) + geom_line(aes(), size=0.5) + geom_abline(intercept = 0) + xlab('False Positive Rate (1 - Specificity)') + ylab('True Positive Rate (Sensitivity)') + annotate("text",x=.7,y=0.95,label=paste('The Area Under the Curve is',auc))
roc.plot
```
