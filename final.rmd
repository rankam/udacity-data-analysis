...
extractFirstName <- function(data){
...
data$contbr_fnm <- extractFirstName(data$contbr_nm)
...
data$predicted_gender <- getFinalGender(data$gender)
...
ggplot(subset(data, cand_party == 'D' | cand_party == 'R'),aes(1, log(contb_receipt_amt+1))) + geom_boxplot(aes(fill=cand_party))
...
ggplot(data, aes(predicted_gender, contb_receipt_amt)) + geom_bar(stat='identity') + facet_wrap(~cand_party)
ggplot(data, aes(1, log(contb_receipt_amt), fill=predicted_gender)) + geom_boxplot() + facet_grid(~cand_party)
```
...
ggplot(data,aes(elect_delta, contb_receipt_amt)) + geom_point(alpha = .1, position= position_jitter(), color="red") + coord_trans(y = "sqrt") + geom_line(stat = "summary", fun.y = median) + ylim(0,quantile(data$contb_receipt_amt, 0.95)) 
```
