...
ggplot(data, aes(x=predicted_gender)) + geom_bar() + facet_wrap(~cand_party)#used
```
...
ggplot(subset(data,cand_party=='D' | cand_party == 'R'), aes(1,log(estimated_salary+10))) + geom_boxplot(aes(fill=cand_party)) #using in bivariate
# relationship between age of topic and length of wikipedia article (count of words)
...
```{r, message=F, warning=F}
data <- subset(data, cand_party == 'D' | cand_party == 'R' )
data.elect <- groupData(data[,1:33], elect_delta,cand_party)
...
ggplot(data,aes(weekday)) + geom_bar() + facet_wrap(~ cand_party)#using


ggplot(data,aes(cand_party,contb_receipt_amt)) + geom_boxplot() + ylim(0, quantile(data$contb_receipt_amt, .95)) + facet_grid(~yearmon)
...
data.cand_party.elect_delta <- groupData(data[,1:33], elect_delta,cand_party)

ggplot(data.cand_party.elect_delta, aes(elect_delta, sqrt(sum_contb))) + geom_point() + geom_smooth(method="lm")

ggplot(data.cand_party.elect_delta, aes(elect_delta, median_contb, color=cand_party)) + geom_point()

data.cand_party.include_gender <- groupData(data[,1:33], included_gender,cand_party)
